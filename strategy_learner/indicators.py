...
def get_bollinger_bands(df_prices, syms):
    avg_val = df_prices.rolling(window=10).mean()
    std_val = df_prices.rolling(window=10).std()
    nom = (df_prices - avg_val)
    denom = (2 * std_val)
    bbp = nom / denom
    return bbp


def get_SMA(df_prices, syms):
    sma = df_prices.rolling(window=10, min_periods=10).mean()
    return sma


def get_momentum(df_prices):
...
def get_rsi(df_prices, syms):
    ...
    rsi_mean_up = delta_up.rolling(window=10).mean()
    rsi_mean_down = delta_down.rolling(window=10).mean()
